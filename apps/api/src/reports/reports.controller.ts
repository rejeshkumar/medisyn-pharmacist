...
export class ReportsController {
  ...
  @Get('gst-summary')
  async getGstSummary(@Query('month') month: string, @Query('year') year: string, @Req() req: any) {
    const tenantId = req.user.tenant_id;
    const m = parseInt(month) || new Date().getMonth() + 1;
    const y = parseInt(year)  || new Date().getFullYear();

    const salesRows = await this.ds.query(`
      SELECT COALESCE(si.gst_percent,0) AS gst_rate,
      ...
      WHERE s.tenant_id=$1 AND EXTRACT(MONTH FROM s.created_at)=$2
        AND EXTRACT(YEAR FROM s.created_at)=$3 AND s.is_voided=false
      GROUP BY si.gst_percent ORDER BY si.gst_percent
    `, [tenantId, m, y]).catch(() => []);

    const clinicRows = await this.ds.query(`
      SELECT COALESCE(cbi.gst_percent,0) AS gst_rate,
      ...
      FROM clinic_bill_items cbi JOIN clinic_bills cb ON cb.id=cbi.bill_id
      WHERE cb.tenant_id=$1 AND EXTRACT(MONTH FROM cb.created_at)=$2
        AND EXTRACT(YEAR FROM cb.created_at)=$3 AND cb.is_voided=false
      GROUP BY cbi.gst_percent ORDER BY cbi.gst_percent
    `, [tenantId, m, y]).catch(() => []);

    const itcRows = await this.ds.query(`
      SELECT COALESCE(poi.gst_percent,0) AS gst_rate,
      ...
      WHERE po.tenant_id=$1 AND EXTRACT(MONTH FROM po.order_date)=$2
    ...
    const breakdown = Object.values(map).sort((a:any,b:any)=>a.gst_rate-b.gst_rate).map((r:any) => ({
      gst_rate: r.gst_rate,
    ...
    const totals = breakdown.reduce((acc,r) => ({
      taxable_value: acc.taxable_value+r.taxable_value, cgst: acc.cgst+r.cgst,
      sgst: acc.sgst+r.sgst, total_gst: acc.total_gst+r.total_gst,
      itc: acc.itc+r.itc, net_gst_payable: acc.net_gst_payable+r.net_gst_payable,
    }), { taxable_value:0,cgst:0,sgst:0,total_gst:0,itc:0,net_gst_payable:0 });
    return { month:m, year:y, breakdown, totals: Object.fromEntries(Object.entries(totals).map(([k,v])=>[k,+Number(v).toFixed(2)])) };
  }

  @Get('schedule-h-register')
  async getScheduleHRegister(@Query('from') from: string, @Query('to') to: string, @Query('schedule') schedule: string, @Req() req: any) {
    const tenantId = req.user.tenant_id;
    ...
    const placeholders = schedules.map((_,i) => `$${i+4}`).join(',');
    const entries = await this.ds.query(`
      SELECT ROW_NUMBER() OVER (ORDER BY s.created_at,si.id) AS entry_no,
      ...
      WHERE s.tenant_id=$1 AND s.is_voided=false
        ...
        AND m.schedule_class::TEXT IN (${placeholders})
      ORDER BY s.created_at,si.id
    `, [tenantId, fromDate, toDate, ...schedules]).catch(() => []);
    return { from: fromDate, to: toDate, schedule: schedule||'H', entries, total: entries.length };
  }

  @Get('ar-aging')
  async getArAging(@Query('as_of') asOf: string, @Req() req: any) {
    const tenantId = req.user.tenant_id;
    const asOfDate = asOf || new Date().toISOString().split('T')[0];
    const rows = await this.ds.query(`
      SELECT s.id, s.bill_number, s.created_at::date AS bill_date,
        COALESCE(p.name,'Walk-in') AS patient_name, COALESCE(p.mobile,'') AS patient_mobile,
        s.total_amount, COALESCE(s.paid_amount,0) AS paid_amount,
      ...
      WHERE s.tenant_id=$2 AND s.is_voided=false AND s.payment_mode='credit'
        AND s.total_amount>COALESCE(s.paid_amount,0) AND s.created_at::date<=$1::date
      ORDER BY s.created_at DESC
    `, [asOfDate, tenantId]).catch(() => []);
    const buckets: Record<string,any[]> = { current:[],days_1_30:[],days_31_60:[],days_61_90:[],over_90:[] };
    for (const r of rows) {
      const d = parseInt(r.days_outstanding)||0;
      const item = {...r, outstanding: parseFloat(r.outstanding)||0};
      if (d<=0) buckets.current.push(item);
      else if (d<=30) buckets.days_1_30.push(item);
      else if (d<=60) buckets.days_31_60.push(item);
      else if (d<=90) buckets.days_61_90.push(item);
      else buckets.over_90.push(item);
    }
    const sum = (arr:any[]) => +arr.reduce((s,r)=>s+r.outstanding,0).toFixed(2);
    return { as_of: asOfDate, summary: { current:sum(buckets.current), days_1_30:sum(buckets.days_1_30), days_31_60:sum(buckets.days_31_60), days_61_90:sum(buckets.days_61_90), over_90:sum(buckets.over_90), total:+rows.reduce((s,r)=>s+(parseFloat(r.outstanding)||0),0).toFixed(2) }, buckets };
  }

  @Get('profit-loss')
  async getProfitLoss(@Query('month') month: string, @Query('year') year: string, @Req() req: any) {
    const tenantId = req.user.tenant_id;
    const m = parseInt(month)||new Date().getMonth()+1;
    const y = parseInt(year)||new Date().getFullYear();
    const q = (sql:string,p:any[]) => this.ds.query(sql,p).catch(()=>[{v:0}]);
    const [[pharma],[clinic],[vip],[cogsRow]] = await Promise.all([
      q(`SELECT COALESCE(SUM(total_amount),0) AS v FROM sales WHERE tenant_id=$1 AND is_voided=false AND EXTRACT(MONTH FROM created_at)=$2 AND EXTRACT(YEAR FROM created_at)=$3`,[tenantId,m,y]),
      q(`SELECT COALESCE(SUM(total_amount),0) AS v FROM clinic_bills WHERE tenant_id=$1 AND is_voided=false AND EXTRACT(MONTH FROM created_at)=$2 AND EXTRACT(YEAR FROM created_at)=$3`,[tenantId,m,y]),
      q(`SELECT COALESCE(SUM(amount_paid),0) AS v FROM vip_registrations WHERE tenant_id=$1 AND EXTRACT(MONTH FROM start_date)=$2 AND EXTRACT(YEAR FROM start_date)=$3`,[tenantId,m,y]),
      q(`SELECT COALESCE(SUM(si.qty*COALESCE(poi.cost_price,si.mrp*0.7)),0) AS v FROM sale_items si JOIN sales s ON s.id=si.sale_id LEFT JOIN inventory_batches ib ON ib.medicine_id=si.medicine_id AND ib.batch_no=si.batch_no LEFT JOIN purchase_order_items poi ON poi.id=ib.purchase_item_id WHERE s.tenant_id=$1 AND s.is_voided=false AND EXTRACT(MONTH FROM s.created_at)=$2 AND EXTRACT(YEAR FROM s.created_at)=$3`,[tenantId,m,y]),
    ]);
    const expRows = await this.ds.query(`SELECT category,COALESCE(SUM(amount),0) AS total FROM expenses WHERE tenant_id=$1 AND EXTRACT(MONTH FROM expense_date)=$2 AND EXTRACT(YEAR FROM expense_date)=$3 AND payment_type!='income' GROUP BY category ORDER BY total DESC`,[tenantId,m,y]).catch(()=>[]);
    const [payroll] = await this.ds.query(`SELECT COALESCE(SUM(net_salary),0) AS v FROM payroll WHERE tenant_id=$1 AND EXTRACT(MONTH FROM pay_date)=$2 AND EXTRACT(YEAR FROM pay_date)=$3`,[tenantId,m,y]).catch(()=>[{v:0}]);
    const rev = { pharmacy:+parseFloat(pharma?.v||0).toFixed(2), consultation:+parseFloat(clinic?.v||0).toFixed(2), vip:+parseFloat(vip?.v||0).toFixed(2), lab:0 };
    const totalRevenue = rev.pharmacy+rev.consultation+rev.vip;
    const cogs = +parseFloat(cogsRow?.v||0).toFixed(2);
    const grossProfit = +(totalRevenue-cogs).toFixed(2);
    const opex = expRows.map((r:any)=>({category:r.category,amount:+parseFloat(r.total).toFixed(2)}));
    const salTotal = +parseFloat(payroll?.v||0).toFixed(2);
    if (salTotal>0) opex.push({category:'Salaries & Wages',amount:salTotal});
    const totalOpex = +opex.reduce((s:number,r:any)=>s+r.amount,0).toFixed(2);
    const netProfit = +(grossProfit-totalOpex).toFixed(2);
    return { month:m, year:y, revenue:{...rev,total:+totalRevenue.toFixed(2)}, cogs, gross_profit:grossProfit, gross_margin_pct:totalRevenue>0?+((grossProfit/totalRevenue)*100).toFixed(1):0, operating_expenses:opex, total_opex:totalOpex, net_profit:netProfit, net_margin_pct:totalRevenue>0?+((netProfit/totalRevenue)*100).toFixed(1):0 };
  }
