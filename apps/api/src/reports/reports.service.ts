...
export class ReportsService {
  ...
  async getDashboard(tenantId: string) {
    ...
    const [todaySales, todayBillCount, lowStockCount, nearExpiryCount] =
      await Promise.all([
        this.saleRepo
          ...
          .andWhere('s.created_at BETWEEN :start AND :end', { start: todayStart, end: todayEnd })
          .andWhere('s.is_voided = false')
          .getRawOne(),

        this.saleRepo.count({
          where: { tenant_id: tenantId, is_voided: false } as any,
        }),

        this.batchRepo
          .createQueryBuilder('b')
          .where('b.tenant_id = :tenantId', { tenantId })
          .andWhere('b.quantity <= 10')
        ...
        this.batchRepo
          .createQueryBuilder('b')
          .where('b.tenant_id = :tenantId', { tenantId })
          .andWhere('b.expiry_date <= :d90', { d90: dayjs().add(90, 'day').toDate() })
    ...
    const topMedicines = await this.saleItemRepo
      ...
      .innerJoin('si.sale', 's', 's.is_voided = false AND s.tenant_id = :tenantId', { tenantId })
  ...
  async getDailySales(tenantId: string, date?: string) {
    ...
    const sales = await this.saleRepo
      ...
      .leftJoinAndSelect('s.pharmacist', 'pharmacist')
      .where('s.tenant_id = :tenantId', { tenantId })
      .andWhere('s.is_voided = false')
    ...
    return { date: targetDate.format('YYYY-MM-DD'), total, sales };
  }

  async getPeriodSales(tenantId: string, from: string, to: string) {
  ...
  async getTopMedicines(tenantId: string, from?: string, to?: string) {
  ...
  async getNearExpiryReport(tenantId: string, days = 90) {
  ...
  async exportSalesToExcel(tenantId: string, from: string, to: string): Promise<Buffer> {
    const sales = await this.getPeriodSales(tenantId, from, to);
