...
export default function ConsultPage() {
  ...
  const loadData = useCallback(async () => {
    if (!queueId) return;
    try {
      const [queueRes, preCheckRes] = await Promise.all([
      ...
      try {
        const consultRes = await api.get(`/consultations/queue/${queueId}`);
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      toast.success(`Prescription ${rxRes.data?.prescription_no || ''} created`);
        ...
        <div className="flex-1">
          ...
          {(patient?.known_allergies || patient?.allergies) && (
            <div className="mt-2 text-xs text-red-600 bg-red-50 border border-red-100 rounded-lg px-3 py-1.5 inline-block">
              ⚠️ Allergies: {patient.known_allergies || patient.allergies}
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-100 p-5 space-y-4">
          ...
          <div className="flex justify-end pt-2">
            <button
              ...
              disabled={!form.diagnosis || saving}
        ...
        <div className="bg-white rounded-xl border border-slate-100 p-5">
            ...
            {items.map((item, i) => (
              <div key={i} className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  ...
                  <input type="text" value={item.dosage} onChange={(e) => updateItem(i, 'dosage', e.target.value)} placeholder="Dosage (e.g. 500mg)" className="text-sm border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#00475a]/20 focus:border-[#00475a] bg-white" />
                  <input type="text" value={item.frequency} onChange={(e) => updateItem(i, 'frequency', e.target.value)} placeholder="Frequency (e.g. TID)" className="text-sm border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#00475a]/20 focus:border-[#00475a] bg-white" />
                  <input type="text" value={item.duration} onChange={(e) => updateItem(i, 'duration', e.target.value)} placeholder="Duration (e.g. 5 days)" className="text-sm border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#00475a]/20 focus:border-[#00475a] bg-white" />
                  <input type="number" value={item.quantity} onChange={(e) => updateItem(i, 'quantity', e.target.value)} placeholder="Qty" className="text-sm border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#00475a]/20 focus:border-[#00475a] bg-white" />
                </div>
                <input type="text" value={item.instructions} onChange={(e) => updateItem(i, 'instructions', e.target.value)} placeholder="Special instructions (e.g. after food)" className="w-full text-sm border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#00475a]/20 focus:border-[#00475a] bg-white" />
              </div>
            ))}
          </div>
          ...
          <div className="flex justify-end">
            ...
            >
              ...
              {finishing ? 'Processing...' : 'Issue Prescription & Complete'}
            </button>
          </div>
        </div>
      )}
