...
import {
  ...
  AlertCircle, Users, TrendingUp, ArrowRight,
} from 'lucide-react';
...
export default function DoctorQueuePage() {
  ...
  const callMutation = useMutation({
    mutationFn: (id: string) => api.patch(`/queue/${id}/status`, { status: 'in_consultation', doctor_id: user?.id }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['doctor-queue'] }); qc.invalidateQueries({ queryKey: ['queue-stats'] }); },
  ...
  const actionable = queue.filter((e: any) => ['precheck_done', 'in_consultation', 'waiting'].includes(e.status));
  const done = queue.filter((e: any) => ['consultation_done', 'completed', 'cancelled', 'no_show'].includes(e.status));
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900">Good morning, Dr. {user?.full_name?.split(' ')[0]} 👋</h1>
            ...
            {actionable.map((entry: any) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Status badge */}
                  <span className={cn('text-xs font-medium px-2.5 py-1 rounded-full border flex-shrink-0', cfg.bg, cfg.color)}>
                    {cfg.label}
                  </span>
            ...
            {done.map((entry: any) => {
              ...
              return (
                <div key={entry.id} className="bg-white rounded-xl border border-slate-100 p-4 flex items-center gap-4 opacity-60">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-slate-700 truncate">{entry.patient?.name || 'Unknown'}</p>
