...
const navItems = [
  ...
  { href: '/nurse/history', label: 'Completed',  icon: ClipboardList },
];
...
export default function NurseLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!roles.some((r: string) => ['nurse', 'owner', 'receptionist'].includes(r))) {
    ...
    <div className="flex h-screen bg-slate-50">
      ...
      <aside className={cn(
        ...
        <div className="p-5 border-b border-slate-100 flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-pink-600 flex items-center justify-center flex-shrink-0">
            <Stethoscope className="w-5 h-5 text-white" />
          </div>
          <div>
            <h1 className="font-bold text-pink-700 text-sm leading-tight">MediSyn</h1>
            <p className="text-xs text-slate-400">Nurse Station</p>
          </div>
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)}
                className={cn('flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all',
                  active ? 'bg-pink-600 text-white' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900')}>
        ...
        <div className="p-3 border-t border-slate-100">
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg mb-1">
            <div className="w-8 h-8 bg-pink-100 rounded-full flex items-center justify-center text-pink-700 font-bold text-sm flex-shrink-0">
              {user?.full_name?.[0]?.toUpperCase() || 'N'}
            </div>
            <div className="min-w-0">
              <p className="text-sm font-medium text-slate-900 truncate">{user?.full_name}</p>
              <p className="text-xs text-slate-400 capitalize">Nurse</p>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="bg-white border-b border-slate-100 px-4 py-3 flex items-center gap-3 lg:hidden">
          <button onClick={() => setSidebarOpen(true)} className="text-slate-500"><Menu className="w-5 h-5" /></button>
          <span className="font-bold text-pink-700 text-sm">Nurse Station</span>
        </header>
