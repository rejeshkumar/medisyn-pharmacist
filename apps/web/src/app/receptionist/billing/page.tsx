...
function AddServiceModal({ onAdd, onClose }: { onAdd: (item: LineItem) => void; onClose: () => void }) {
  ...
  const filtered = catalog.filter(s => !search || s.name.toLowerCase().includes(search.toLowerCase()));

  const addFromCatalog = (svc: any) => {
    onAdd({ category, name: svc.name, qty: 1, unit_rate: n(svc.rate), gst_percent: n(svc.gst_percent) });
    onClose();
  };

  const addCustom = () => {
    if (!customName.trim() || !customRate) { toast.error('Name and rate are required'); return; }
    onAdd({ category, name: customName.trim(), qty: 1, unit_rate: n(customRate), gst_percent: n(customGst) });
    onClose();
  };
        ...
        <div className="flex-1 overflow-y-auto p-3">
          {!custom ? (
            <>
              ...
              {filtered.length === 0 && <p className="text-sm text-slate-400 text-center py-4">No services found</p>}
            ...
            <div className="space-y-3 p-1">
              <button onClick={() => setCustom(false)} className="text-xs text-[#00475a]">← Back to catalog</button>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" min="0" value={customRate} onChange={e => setCustomRate(e.target.value)}
                    className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-[#00475a]" />
                </div>
                <div>
                  <label className="text-xs text-slate-500 mb-1 block">GST %</label>
                  <input type="number" min="0" max="28" value={customGst} onChange={e => setCustomGst(e.target.value)}
                    className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-[#00475a]" />
                </div>
              </div>
...
export default function ReceptionistBillingPage() {
  ...
  // ── View state ──────────────────────────────────────────────
  const [view, setView] = useState<'today' | 'billing' | 'done'>('today');
  ...
  const selectPatient = async (p: any) => {
    setSelectedPatient(p); setPatients([]); setPatientSearch('');
    try { const r = await api.get(`/patients/${p.id}/vip`); setVipInfo(r.data); }
    ...
    try {
      ...
      const svcItems: LineItem[] = (r.data.services || [])
        ...
        .map((s: any) => ({
          ...
          source: 'encounter_service', source_id: s.id,
        }));
      if (r.data.summary?.consultation_fee > 0) {
        svcItems.unshift({
          category: 'consultation',
          name: `Consultation — ${r.data.queue?.visit_type || 'new visit'}`,
          qty: 1, unit_rate: Number(r.data.summary.consultation_fee),
          gst_percent: 0, source: 'consultation',
        });
      }
      setItems(svcItems);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await api.post('/clinic-bills', {
        patient_id: selectedPatient.id, items, payment_mode: paymentMode,
  ...
  const reset = () => {
    setDoneBill(null); setSelectedPatient(null); setVipInfo(null); setItems([]);
    setPatientSearch(''); setExtraDiscountPct(''); setExtraDiscountNote('');
    setNotes(''); setPaymentMode('cash'); setSelectedEncounter(null);
    setEncounterDetail(null); setView('today');
  };
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-8 text-center shadow-sm">
          ...
          <div className="bg-slate-50 rounded-xl p-4 text-left space-y-2 mb-6 text-sm">
            ...
            {n(doneBill.vip_discount_amount) > 0 && (
              <div className="flex justify-between text-amber-600"><span>VIP discount</span><span>−₹{fmt(doneBill.vip_discount_amount)}</span></div>
            )}
            {n(doneBill.extra_discount_amt) > 0 && (
              <div className="flex justify-between text-green-600"><span>Extra discount</span><span>−₹{fmt(doneBill.extra_discount_amt)}</span></div>
            )}
            <div className="flex justify-between font-bold text-base border-t border-slate-200 pt-2 mt-1">
              <span>Total paid</span><span className="text-[#00475a]">₹{fmt(doneBill.total_amount)}</span>
            </div>
          </div>
      ...
      <div className="p-6 max-w-3xl">
        ...
        {loadingEncounter ? (
          ...
          <div className="space-y-2">
            {(todayEncounters as any[]).length === 0 ? (
              <div className="text-center py-16 text-slate-400">
                <p className="text-sm">No patients today yet</p>
              </div>
            ) : (todayEncounters as any[]).map((enc: any) => {
              const pending = Number(enc.pending_services || 0);
              const clinicTotal = Number(enc.consultation_fee || 0) + Number(enc.services_total || 0);
              return (
                <button key={enc.queue_id} onClick={() => loadEncounterDetail(enc.queue_id)}
                  ...
                  <div className="flex items-center justify-between text-xs text-slate-500">
                    <div className="flex gap-3">
                      ...
                      {Number(enc.pharmacy_total) > 0 && (
                        <span>Pharmacy: <strong className="text-slate-700">₹{Number(enc.pharmacy_total).toFixed(0)}</strong></span>
                      )}
                      {pending > 0 && (
  ...
  return (
    <div className="p-6 max-w-3xl">
      <div className="flex items-center gap-3 mb-5">
        <button onClick={reset} className="text-slate-400 hover:text-slate-600 text-sm flex items-center gap-1">
          ← Back
        </button>
        <h1 className="text-xl font-bold text-slate-900">
          {selectedEncounter ? encounterDetail?.queue?.patient_name || 'Patient bill' : 'Manual bill'}
        </h1>
      </div>

      {/* Pharmacy bill info */}
      {encounterDetail?.pharmacy_bills?.length > 0 && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-3 mb-3 flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold text-green-800">Pharmacy bill generated separately</p>
              ...
              <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-100 rounded-xl shadow-lg z-10 overflow-hidden">
                {patients.map(p => (
                  <button key={p.id} onClick={() => selectPatient(p)}
                    ...
                    {p.vip_tier && <Star className="w-3.5 h-3.5 text-amber-500 ml-auto" />}
                  </button>
                ))}
              </div>
            )}
        ...
        return (
          <div key={cat} className="bg-white rounded-xl border border-slate-100 p-4 mb-3">
            <div className="flex items-center gap-2 mb-3">
              <div className={`p-1.5 rounded-lg ${meta.color}`}><Icon className="w-3.5 h-3.5" /></div>
        ...
        <div className="bg-white rounded-xl border border-slate-100 p-5">
          <div className="space-y-2 mb-4">
            ...
            <div className="flex items-center gap-2 pt-1">
              ...
              <input type="number" min="0" max="100" value={extraDiscountPct}
              ...
              <input type="text" value={extraDiscountNote} onChange={e => setExtraDiscountNote(e.target.value)}
