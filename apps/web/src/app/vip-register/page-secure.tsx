...
Valid Until: ${patient?.vip_end_date ? new Date(patient.vip_end_date).toLocaleDateString('en-IN') : 'One Year'}
...
MediSyn Specialty Clinic, Taliparamba`;
  ...
  // Show error if no agent/token in URL
  if (!agentCode || !accessToken) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-50 via-white to-red-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl border border-red-200 p-8 max-w-md text-center">
          ...
          <p className="text-gray-600 text-sm">
            This VIP registration link is invalid or incomplete. Please contact your sales agent for the correct link.
          </p>
        </div>
      </div>
    );
