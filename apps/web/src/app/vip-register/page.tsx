...
export default function VipRegisterPage() {
  ...
  const [form, setForm] = useState({
    ...
    mobile: '', vip_registered_by: '', vip_category: 'individual',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.vip_registered_by) { setError('Please select who is selling this pass.'); return; }
    setError(''); setLoading(true);
    try {
      const { data } = await api.post('/patients/vip-register', form);
      setPatient(data); setStep('success');
    } catch (err: any) {
      ...
      <div className="flex gap-3 mb-5 flex-wrap justify-center">
        {['Priority Service', 'Exclusive Discounts', 'Health Reminders'].map(b => (
          <div key={b} className="flex items-center gap-1.5 bg-white border border-amber-200 rounded-full px-3 py-1.5 text-xs font-medium text-amber-700 shadow-sm">
            <Star className="w-3 h-3 fill-amber-400 text-amber-400" />{b}
          </div>
        ))}
      </div>
        ...
        {step === 'form' ? (
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-xl border border-amber-100 overflow-hidden">
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <div className="flex-1">
                  ...
                  <input className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400"
                    value={form.first_name} onChange={e => set('first_name', e.target.value)} placeholder="First name" required />
                </div>
                <div className="flex-1">
              ...
              <div>
                ...
                <select className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400"
                  value={form.vip_registered_by} onChange={e => set('vip_registered_by', e.target.value)}>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-800 flex items-center gap-2">
                ...
                <span>Valid for <strong>1 full year</strong> until <strong>{vipEndDate}</strong></span>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-amber-100 overflow-hidden text-center">
            ...
            <div className="p-5 space-y-4">
              <div className="bg-gradient-to-br from-amber-400 to-yellow-500 rounded-2xl p-5 text-white shadow-lg">
                ...
                <div className="mt-3 pt-3 border-t border-amber-300 flex justify-between items-end">
                  ...
                  <div className="text-right">
                    ...
                    <p className="font-bold text-sm capitalize">{patient?.vip_category || form.vip_category}</p>
              ...
              <button
                onClick={() => { setStep('form'); setForm({ salutation: 'Mr', first_name: '', last_name: '', mobile: '', vip_registered_by: '', vip_category: 'individual' }); setPatient(null); }}
