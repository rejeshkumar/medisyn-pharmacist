...
import { Clock, Plus, Trash2, Loader2, CalendarX, CheckCircle, XCircle } from 'lucide-react';
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-10 h-10 rounded-full bg-teal-100 flex items-center justify-center text-[#00475a] font-bold">
          {doctorName[0]?.toUpperCase()}
        </div>
        <div>
          <h2 className="font-bold text-slate-800">Dr. {doctorName}</h2>
          <p className="text-xs text-slate-400">Availability Schedule</p>
        </div>
      </div>
      ...
      {tab === 'schedule' && (
        <div className="space-y-2">
          {[1, 2, 3, 4, 5, 6, 0].map(day => {
            ...
            return (
              <div key={day} className="bg-white rounded-xl border border-slate-100 shadow-sm p-4">
                <div className="flex items-center gap-3">
                  <div className={`w-10 text-center text-xs font-bold py-1 rounded-lg ${existing?.is_active ? 'bg-teal-50 text-[#00475a]' : 'bg-slate-50 text-slate-400'}`}>
                    {DAY_SHORT[day]}
                  </div>
                  ...
                  {existing ? (
                    <div className="flex-1 flex items-center gap-3 flex-wrap">
                      {canEdit ? (
                        <>
                          <input type="time" value={existing.start_time}
                            onChange={e => saveDay(day, { ...existing, start_time: e.target.value })}
                            className="text-sm border border-slate-200 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-[#00475a]/20" />
                          <span className="text-slate-400 text-sm">to</span>
                          <input type="time" value={existing.end_time}
                            onChange={e => saveDay(day, { ...existing, end_time: e.target.value })}
                            className="text-sm border border-slate-200 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-[#00475a]/20" />
                          <select value={existing.slot_duration_mins}
                            onChange={e => saveDay(day, { ...existing, slot_duration_mins: parseInt(e.target.value) })}
                            className="text-sm border border-slate-200 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-[#00475a]/20">
                          ...
                          <select value={existing.max_patients_per_slot}
                            onChange={e => saveDay(day, { ...existing, max_patients_per_slot: parseInt(e.target.value) })}
                            className="text-sm border border-slate-200 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-[#00475a]/20">
                        ...
                        <span className="text-sm text-slate-600">
                          {existing.start_time} – {existing.end_time} · {existing.slot_duration_mins} min slots · {existing.max_patients_per_slot} patient/slot
                        </span>
                      )}
                    ...
                    <div className="flex-1 flex items-center justify-between">
                      <span className="text-sm text-slate-400 flex items-center gap-1"><XCircle className="w-3 h-3" />Not available</span>
                      {canEdit && (
                        <button onClick={() => saveDay(day, {})} disabled={isSaving}
                          className="flex items-center gap-1 text-xs text-[#00475a] border border-[#00475a]/30 px-2 py-1 rounded-lg hover:bg-teal-50 transition-colors disabled:opacity-50">
          ...
          {canEdit && (
            <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 mb-4">
              <h3 className="text-sm font-semibold text-slate-700 mb-3 flex items-center gap-2">
                <CalendarX className="w-4 h-4 text-[#00475a]" />Add Leave / Off Day
              </h3>
              <div className="flex gap-3 flex-wrap">
                ...
                <button onClick={addLeave} disabled={addingLeave}
                  className="flex items-center gap-2 bg-[#00475a] text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-[#00475a]/90 disabled:opacity-50 transition-colors">
            ...
            <div className="space-y-2">
              {leaves.map(l => (
                <div key={l.id} className="bg-white rounded-xl border border-slate-100 shadow-sm px-4 py-3 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-slate-800">{format(new Date(l.leave_date), 'dd MMM yyyy (EEEE)')}</p>
                    {l.reason && <p className="text-xs text-slate-400">{l.reason}</p>}
                  </div>
                  {canEdit && (
                    <button onClick={() => removeLeave(l.id)} className="text-slate-400 hover:text-red-500 transition-colors">
