// apps/web/app/(dashboard)/inventory/intelligence/config/page.tsx
'use client';
    ...
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-2">
          Configure thresholds for movement categorization and stock alerts
        </p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-8">
        ...
        <div className="border-b border-gray-200 pb-6">
          ...
          <div className="mt-4">
            ...
            <input
              type="text"
              value={config.fast_moving_description}
              onChange={(e) => handleChange('fast_moving_description', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#00475a]"
            />
          </div>
          <p className="text-sm text-gray-500 mt-2">
            Current setting: Medicines sold <strong>{config.fast_moving_sales_count}+</strong> times in <strong>{config.fast_moving_days}</strong> days
          </p>
        </div>
        ...
        <div className="border-b border-gray-200 pb-6">
          ...
          <div className="mt-4">
            ...
            <input
              type="text"
              value={config.slow_moving_description}
              onChange={(e) => handleChange('slow_moving_description', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#00475a]"
            />
          </div>
          <p className="text-sm text-gray-500 mt-2">
            Current setting: Medicines sold <strong>{config.slow_moving_sales_count_min}-{config.slow_moving_sales_count_max}</strong> times in <strong>{config.slow_moving_days}</strong> days
          </p>
        </div>
        ...
        <div className="pb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                type="text"
                value={config.dead_stock_description}
                onChange={(e) => handleChange('dead_stock_description', e.target.value)}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#00475a]"
              />
            </div>
          </div>
          <p className="text-sm text-gray-500 mt-2">
            Current setting: No sales in <strong>{config.dead_stock_days}</strong> days
          </p>
        </div>
        ...
        <div className="flex justify-end space-x-4 pt-6 border-t border-gray-200">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="px-6 py-2 bg-[#00475a] text-white rounded-md hover:bg-[#003845] disabled:opacity-50 disabled:cursor-not-allowed"
          >
