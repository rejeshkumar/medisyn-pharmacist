// apps/api/src/inventory/services/inventory-intelligence.service.ts
...
export class InventoryIntelligenceService {
  ...
  async generateAIPrediction(
    ...
    // Fetch medicine data and velocity
    ...
    // Fetch historical sales pattern (last 90 days)
    const salesHistory = await this.dataSource.query(
      `SELECT 
        DATE(b.created_at + INTERVAL '5 hours 30 minutes') as sale_date,
        COUNT(*) as sale_count,
        SUM(bi.quantity) as total_quantity
      FROM bill_items bi
      JOIN bills b ON bi.bill_id = b.id
      WHERE bi.medicine_id = $1 
        AND b.tenant_id = $2
        AND b.status = 'PAID'
        AND b.created_at >= CURRENT_TIMESTAMP - INTERVAL '90 days'
      GROUP BY DATE(b.created_at + INTERVAL '5 hours 30 minutes')
